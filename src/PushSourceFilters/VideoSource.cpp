...
// Constructor
//
CVideoSourcePin::CVideoSourcePin(HRESULT* phr, CSource* pFilter)
      : CPushPin(NAME("Video Source"), phr, pFilter, L"Video")
{
...
// alignment, etc.
HRESULT CVideoSourcePin::DecideBufferSize(IMemAllocator *pAlloc, ALLOCATOR_PROPERTIES *pRequest)
{
    HRESULT hr = S_OK;
...
// in the list.
//
STDMETHODIMP CVideoSourcePin::GetFormat(AM_MEDIA_TYPE** ppamt)
{
	HRESULT hr = S_OK;
...
// SetFormat
//
STDMETHODIMP CVideoSourcePin::SetFormat(AM_MEDIA_TYPE* pamt)
{
	HRESULT hr = S_OK;
...
// CreateInstance
//
CUnknown* WINAPI CVideoSource::CreateInstance(IUnknown* pUnk, HRESULT* phr)
{
	CVideoSource *pNewFilter = new CVideoSource(pUnk, phr);
...
// Constructor
//
CVideoSource::CVideoSource(IUnknown* pUnk, HRESULT* phr)
           : CSource(NAME("Video Push Source"), pUnk, CLSID_VideoSource)
{
	// The pin magically adds itself to our pin array.
	m_pPin = new CVideoSourcePin(phr, this);
...
STDMETHODIMP CVideoSource::GetState(DWORD dwMilliSecsTimeout, FILTER_STATE* pState)
{
    HRESULT hr = S_OK;
