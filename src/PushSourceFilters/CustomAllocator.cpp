...
CCustomAllocator::CCustomAllocator(TCHAR *pName, LPUNKNOWN pUnk, HRESULT *phr, BOOL bEvent, BOOL fEnableReleaseCallback)
	...
	, m_lDeliver("Push source custom allocator deliver list", SAMPLE_LIST_SIZE)
	, m_lBuffers("Push source custom allocator buffer list", SAMPLE_LIST_SIZE)
	, m_hSemFree(NULL)
	, m_lWaitingFree(0)
{
...
STDMETHODIMP CCustomAllocator::NonDelegatingQueryInterface(REFIID riid, void** ppv)
{
	...
	if (riid == IID_IPushSource2)
	{
		return GetInterface((IPushSource2*)this, ppv);
	}
