...
// Constructor
//
CAudioSourcePin::CAudioSourcePin(HRESULT* phr, CSource* pFilter)
	: CPushPin(NAME("Audio Source"), phr, pFilter, L"Audio")
	, m_rtMediaTime(0)
{
...
// alignment, etc.
HRESULT CAudioSourcePin::DecideBufferSize(IMemAllocator *pAlloc, ALLOCATOR_PROPERTIES *pRequest)
{
    HRESULT hr = S_OK;
...
HRESULT CAudioSourcePin::FillBuffer(IMediaSample *pSample)
{
...
// OnThreadStartPlay
//
HRESULT CAudioSourcePin::OnThreadStartPlay(void)
{
	m_rtMediaTime = 0;
	return CPushPin::OnThreadStartPlay();
}
...
// in the list.
//
STDMETHODIMP CAudioSourcePin::GetFormat(AM_MEDIA_TYPE** ppamt)
{
	HRESULT hr = S_OK;
...
// SetFormat
//
STDMETHODIMP CAudioSourcePin::SetFormat(AM_MEDIA_TYPE* pamt)
{
	HRESULT hr = S_OK;
...
// CreateInstance
//
CUnknown* WINAPI CAudioSource::CreateInstance(IUnknown* pUnk, HRESULT* phr)
{
	CAudioSource *pNewFilter = new CAudioSource(pUnk, phr);
...
// Constructor
//
CAudioSource::CAudioSource(IUnknown* pUnk, HRESULT* phr)
           : CSource(NAME("0Audio Push Source"), pUnk, CLSID_AudioSource)
{
	// The pin magically adds itself to our pin array.
	m_pPin = new CAudioSourcePin(phr, this);
...
//
STDMETHODIMP CAudioSource::NonDelegatingQueryInterface(REFIID riid, void** ppv)
{
...
STDMETHODIMP CAudioSource::GetState(DWORD dwMilliSecsTimeout, FILTER_STATE* pState)
{
    HRESULT hr = S_OK;
