...
// Constructor
//
CPushPin::CPushPin(TCHAR* pObjectName, HRESULT* phr, CSource* pFilter, LPCWSTR pName)
...
{
	if ((riid == IID_IPushSource) || (riid == IID_IPushSource2))
	{
		if (m_pAllocator)
		{
...
//
HRESULT CPushPin::InitAllocator(IMemAllocator** ppAllocator)
{
...
// in the list.
//
STDMETHODIMP CPushPin::GetFormat(AM_MEDIA_TYPE** ppamt)
{
	HRESULT hr = S_OK;
...
// SetFormat
//
STDMETHODIMP CPushPin::SetFormat(AM_MEDIA_TYPE* pamt)
{
	HRESULT hr = S_OK;
...
//
STDMETHODIMP CPushPin::GetStreamCaps(int iIndex, AM_MEDIA_TYPE** ppamt, BYTE* pSCC)
{
	HRESULT hr = S_OK;
	CheckPointer(ppamt, E_POINTER);
...
//
STDMETHODIMP CPushPin::SuggestAllocatorProperties(const ALLOCATOR_PROPERTIES* pprop)
{
	HRESULT hr = S_OK;
	if (pprop)
	{
...
//
STDMETHODIMP CPushPin::GetAllocatorProperties(ALLOCATOR_PROPERTIES* pprop)
{
	HRESULT hr = S_OK;
	if (pprop)
	{
...
CPushSource::CPushSource(IUnknown* pUnk, HRESULT* phr)
           : CSource(NAME("Push Source"), pUnk, CLSID_PushSource)
{
	// The pin magically adds itself to our pin array.
	m_pPin = new CPushPin(NAME("Push Source Pin"), phr, this, L"Out");
...
STDMETHODIMP CPushSource::GetState(DWORD dwMilliSecsTimeout, FILTER_STATE* pState)
{
    HRESULT hr = S_OK;
